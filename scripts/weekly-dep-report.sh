...
# expects a parameter that is the name of the default branch, i.e., "main"
starter_point=${1:-"HEAD"}
...
get_total_deps () {
    echo "$(cargo guppy resolve-cargo --resolver-version v2 --target-platform x86_64-unknown-linux-gnu --host-platform x86_64-unknown-linux-gnu --build-kind target --kind thirdparty --omit-edges-into diem-workspace-hack | wc -l | tr -d ' ')"
}

get_total_direct_deps() {
    echo "$(cargo guppy resolve-cargo --resolver-version v2 --target-platform x86_64-unknown-linux-gnu --host-platform x86_64-unknown-linux-gnu --build-kind target --kind directthirdparty --omit-edges-into diem-workspace-hack | wc -l | tr -d ' ')"
}

get_total_dsr_deps () {
    echo "$(cargo guppy resolve-cargo --resolver-version v2 --target-platform x86_64-unknown-linux-gnu --host-platform x86_64-unknown-linux-gnu --build-kind target --kind thirdparty --omit-edges-into diem-workspace-hack -p safety-rules | wc -l | tr -d ' ')"
}

get_total_direct_dsr_deps () {
    echo "$(cargo guppy resolve-cargo --resolver-version v2 --target-platform x86_64-unknown-linux-gnu --host-platform x86_64-unknown-linux-gnu --build-kind target --kind directthirdparty --omit-edges-into diem-workspace-hack -p safety-rules | wc -l | tr -d ' ')"
}

get_total_dups () {
    echo "$(cargo guppy dups --target x86_64-unknown-linux-gnu | wc -l | tr -d ' ')"
}
...
echo "Dependency change in diem/diem:$starter_point at Commit $short_sha ($date) since Commit $prev_short_sha ($prev_date) :"
